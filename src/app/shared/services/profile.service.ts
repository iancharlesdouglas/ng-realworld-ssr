...
export class ProfileService {
	...
	getArticles(username: string, feed: Feed, page: number, pageSize: number): Observable<ArticlesApiResponse> {
		const userParameter = feed === Feed.authored ? `author=${username}` : `favorited=${username}`;
		const url = `${environment.remoteApiHost}/api/articles?offset=${page * pageSize}&limit=${pageSize}&${userParameter}`;
