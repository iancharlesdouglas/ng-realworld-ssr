...
export class HomeComponent implements OnInit, OnDestroy {
	...
	constructor(
		...
		this.routeSub = this.activatedRoute.queryParamMap
			.pipe(
				map(parameters => {
					if (parameters.has(filterParameter) && Object.keys(Feed).includes(parameters.get(filterParameter)!)) {
						return { feed: parameters.get(filterParameter) as Feed, tag: parameters.get('tag') as string } as
							| ActiveFeed
							| undefined;
					}
					return;
	...
	private async getArticles() {
		this.pageSub = combineLatest([this.page$, this.feed$])
			.pipe(
				distinctUntilChanged(),
				tap(async ([page, feed]) => {
					if (feed) {
						const articles = await firstValueFrom(
							this.homeService.getArticles(feed.feed, page, this.pageSize, feed.tag).pipe(
								tap(response => {
									if (response) {
										this.pages$ = range(0, Math.floor((response.articlesCount - 1) / this.pageSize) + 1).pipe(
											toArray(),
										);
									}
								}),
	...
	async favoriteArticle(article: Article): Promise<void> {
		...
		if (user) {
			const articles = await firstValueFrom(
				this.articleService.favoriteArticle(article).pipe(
	...
	async unfavoriteArticle(article: Article): Promise<void> {
		const articles = await firstValueFrom(
			this.articleService.unfavoriteArticle(article).pipe(
				map(() => this.fetchArticles()),
				concatAll(),
			),
		);
	...
	private fetchArticles() {
		...
		if (feed) {
			return this.homeService.getArticles(feed, this.page!, this.pageSize, undefined, true).pipe(
				tap(response => {
					this.pages$ = range(0, Math.floor((response.articlesCount - 1) / this.pageSize) + 1).pipe(toArray());
				}),
				map(response => response.articles),
			);
		}
