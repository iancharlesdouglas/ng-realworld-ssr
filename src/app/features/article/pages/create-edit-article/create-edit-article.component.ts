...
export class CreateEditArticleComponent implements OnDestroy {
	...
	constructor(
		...
		this.articleSub = combineLatest([this.activatedRoute.params, this.stateService.user$])
			.pipe(
				map(([parameters]) => {
					this.articleSlug = parameters['id'];
					if (this.articleSlug) {
	...
	async removeTag(tag: string): Promise<void> {
		const withTagRemoved = {
			...this.article,
			tagList: this.article!.tagList.filter(existTag => existTag !== tag),
		} as CreateEditArticle;
	...
	async addTag(event: KeyboardEvent): Promise<void> {
		if (event.key === 'Enter' || event.key === 'NumpadEnter' || event.key === ' ') {
			...
			const withTagAdded = {
				...this.article,
				tagList: [...this.article!.tagList.filter(existTag => existTag !== tag), tag],
			} as CreateEditArticle;
	...
	async attemptPublish(): Promise<void> {
		...
		if (this.form.valid) {
			...
			this.article = await (this.article?.slug
				? firstValueFrom(this.articleService.updateArticle(this.article))
				: firstValueFrom(this.articleService.createArticle(this.article)));
