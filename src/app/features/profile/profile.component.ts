...
export class ProfileComponent implements OnInit, OnDestroy {
	...
	constructor(
		...
		this.profile$ = this.activatedRoute.params.pipe(
			map(parameters => this.profileService.find(`${parameters['username']}`)),
			concatAll(),
		);
			...
			?.pipe(
				map(parameters => {
					if (parameters.has(filterParameter) && Object.keys(Feed).includes(parameters.get(filterParameter)!)) {
						return parameters.get(filterParameter) as Feed;
					}
					return;
				}),
				tap(feed => {
	...
	private async getArticles() {
		...
		this.pageSub = combineLatest([this.profile$, this.page$, this.feed$])
			.pipe(
				distinctUntilChanged(),
				tap(async ([profile, page, feed]) => {
					if (feed) {
						const articles = await firstValueFrom(
							this.profileService.getArticles(profile.username, feed, page, this.pageSize).pipe(
								tap(response => {
